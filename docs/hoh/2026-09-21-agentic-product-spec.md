...
| A7 | The actual EXE passes visible-control journeys for setup, permission, mission, claim and export at desktop and narrow sizes | unverified | each slice, 6 | partly verified after slices 1–5 — setup (1), permission (3), mission, interruption, pause and claim review (4), diagnostics and export with the archive verified from disk (5) on the desktop binary at desktop size; narrow-size captures exist for slice 1 only; downloads need the UI Automation driver (a CDP click produces none) |
| A8 | Clean-profile package, security/failure paths, performance measurements and live probes before any production claim | unverified | 5, 6 | partly verified after slice 5 — [record](2026-09-21-slice5-export-diagnostics-record.md): package and storage facts, failure paths (broken chain, missing store, failed render, refused download) and the redacted report; clean-profile package and performance measurements are slice 6; no production claim is made |
