...
| A7 | The actual EXE passes visible-control journeys for setup, permission, mission, claim and export at desktop and narrow sizes | unverified | each slice, 6 | partly verified after slices 1–4 — setup (1), permission (3), mission, interruption, pause and claim review (4) on the EXE at desktop size; narrow-size captures exist for slice 1 only; export is slice 5 |
...
| J6 | Claim review: support and contradiction, hashes, unsupported scope, next test; a stale/failed check blocks export; export and replay with limitations intact | unverified | 4, 5 | partly verified after slice 4 — cards show support and contradiction, digests, unsupported scope and next tests; stale checks block export (reviewer: verify, resume, `GET /capsule` 409); export and replay with the new members are slice 5 |
| J7 | Cross-workspace: Memory links to a run; Molecules/BioArt outputs enter claims only through explicit admission; Prose keeps protected spans; Diagnostics agrees with the header; keyboard and 700 px | unverified | 1, 4, 5 | partly verified after slices 1 and 4 — Diagnostics/header agreement, 700 px native and browser, Settings keyboard order, the last mission reopened from the profile; Memory's link to a run deferred (needs main.jsx); the rest in slice 5 |
