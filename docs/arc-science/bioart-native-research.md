...
The native implementation initializes a strict project-local TOML file, inspects configuration, reports local executable presence and launches `python -m arc_science` through an argument vector. It has no startup installation, credential fields or public-listener default. The explicit BioArt environment bridge reaches the Python provider. `doctor` does not probe package imports or certify dependency execution. [Native setup and limits](../../native/arc-science/README.md).

Cancellation covers the worker's process group/job as well as the immediate Python process. The implementation pins `process-wrap` 9.0.0, whose reported minimum Rust version1.86 is compatible with the pinned1.90 toolchain. Its standard-process wrappers provide Unix process groups and Windows job objects. Linux tests exercise cooperative cancellation, forced termination and cleanup after the leader exits; Windows/macOS execution remains pending. The direct worker is reaped, while forced orphan-grandchild reaping belongs to the OS adopter. [process-wrap API](https://docs.rs/process-wrap).
