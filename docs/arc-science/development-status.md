...
| Prose control | Rule-based local rewrite (`arc-prose-rules-1`) outside protected spans (`arc-prose-protect-1`), refused atomically when a protected span would change, idempotent, no egress; third-party detection reproducing the text2go reference request to `api.edgeshop.ai` under per-request consent, a service switch, bounds, a streamed response cap, single flight and a keyed-hash audit; a fifth workspace; neither result is an authorship or validity claim | The live endpoint's response shape (unreachable from this workstation through the proxy); the rule table is deliberately small and mechanical |
