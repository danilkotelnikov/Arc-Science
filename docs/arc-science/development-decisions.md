...
15. Build the native supervisor in the focused Linux Python CI job and run the real renderer-lifetime integration there. Without that build, a clean checkout explicitly skips three native cases. Cost: extra Linux CI build time; Windows/macOS renderer qualification still needs separate execution.
16. Keep final-review finding I1 open and deliver only an unmerged development checkpoint after the one final fix wave. Repeated native cancellation can still kill Python before its detached-renderer cleanup; disclosure is not proof of containment. Cost: native-supervised rendering and merge qualification remain deferred. Configuration, offline BioArt and tested cooperative paths retain their narrower evidence; no all-findings-fixed claim is made.
17. Address I1 by making containment ownership an explicit, versioned native-to-Python contract. Native overrides the inherited marker; on POSIX Python also requires that it is the actual process-group leader. A supervised renderer stays in the outer group, while standalone rendering retains its private session and group cleanup. Cost: the contract is intentionally internal and both halves must be shipped together; deliberately escaping descendants remain outside the trusted-worker boundary.
18. Keep I1 qualification open until a rebuilt Rust binary passes the uninjected double-signal test. The local runtime has no Rust toolchain and GitHub refused development-ref creation with HTTP 403, so the retained older binary was used only with an explicit marker to exercise the Python/process-graph half. Cost: the source fix is preserved without an unsupported merge-ready claim.
