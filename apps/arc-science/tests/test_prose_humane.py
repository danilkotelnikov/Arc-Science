...
def test_instructions_that_ask_for_evasion_or_impersonation_are_refused_before_any_text_leaves():
    ...
    assert refused('make it undetectable by GPTZero') == 'undetectable'
    assert refused('please bypass the AI detector') == 'bypass the AI detector'
    assert refused('write exactly like Professor Smith') == 'write exactly like Professor Smith'
    assert refused('get past Turnitin') == 'get past Turnitin'
    ...
    assert error.value.code == 'refused_instruction' and list(error.value.spans) == [{'change': 'instruction', 'class': 'refused', 'literal': 'read as human written'}]
...
def test_service_routes_diagnose_locally_and_rewrite_through_the_configured_prose_seat(tmp_path, monkeypatch):
    ...
    with TestClient(service.create_app(data_dir=tmp_path / 'data', token=TOKEN)) as c:
        ...
        # An evasion instruction is refused before the seat is started, and audited as such.
        mode['value'] = 'never-run'
        evade = c.post('/api/prose/humanise', headers=AUTH, json={'text': SAMPLE, 'allow_egress': True, 'instructions': 'make it undetectable'})
        ...
        assert [r['status'] for r in records] == ['attempted', 'edited', 'attempted', 'preservation_failed', 'attempted', 'refused_instruction']
...
def test_an_api_key_prose_seat_is_called_over_http_with_the_behaviour_as_instructions(tmp_path, monkeypatch):
    ...
    with TestClient(service.create_app(data_dir=data, token=TOKEN)) as c:
        ...
        settings.replace(doc, snap['revision'])
        assert c.get('/api/capabilities', headers=AUTH).json()['prose_seat'] == {'configured': True, 'provider': 'openai', 'transport': 'api', 'model': 'gpt-5.6-mini'}
        # No stored credential of that name: refused without any request leaving.
        missing = c.post('/api/prose/humanise', headers=AUTH, json={'text': SAMPLE, 'allow_egress': True})
        assert missing.status_code == 502 and 'prose-key' in missing.json()['detail']['detail'] and requests == []
        ...
        assert [r['status'] for r in records] == ['attempted', 'provider_rejected', 'attempted', 'edited']
