...
test('updates contact overlays without clearing the loaded structure or resetting the camera', async () => {
  ...
  await screen.findByText(/2 contact residues from the provisional scene highlighted; coordinates unchanged/);
  expect(context.clear).toHaveBeenCalledTimes(1);
  expect(PluginCommands.Camera.Reset).toHaveBeenCalledTimes(1);
  ...
  await screen.findByText(/4 contact residues from the verified scene highlighted; coordinates unchanged/);
  expect(context.clear).toHaveBeenCalledTimes(1);
...
test('stale contact overlay work cannot add an obsolete representation', async () => {
  ...
  await screen.findByText(/2 contact residues from the verified scene highlighted; coordinates unchanged/);
  ...
  expect(screen.getByRole('status')).not.toHaveTextContent('provisional scene');
});
