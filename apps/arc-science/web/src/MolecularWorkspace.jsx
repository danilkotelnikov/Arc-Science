...
export default function MolecularWorkspace({token, setToken}) {
  ...
  useEffect(() => {
    ...
    apiFetch('/api/settings', {token, signal: controller.signal})
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const response = await apiFetch('/api/molecular/renders/' + renderJob.id + '/source', {token, signal: controller.signal});
  ...
  useEffect(() => {
    ...
    const controller = new AbortController();
    apiFetch('/api/molecular/renders/' + renderJob.id + '/scene', {token, signal: controller.signal})
