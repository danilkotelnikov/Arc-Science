...
export default function MolecularWorkspace() {
  ...
  return <div className="molecular-workspace">
    <aside className="inspector" aria-label="Molecular example details">
      <p className="eyebrow">EXAMPLE / 01</p><h1>{example.title}</h1>
      <a href={example.source.url} target="_blank" rel="noreferrer">{example.source.id} ↗ RCSB PDB</a>
      <dl><dt>Structure</dt><dd>Model {example.source.model} · Assembly {example.source.assembly}</dd>
      ...
      <details className="inspector-section"><summary>Render your structure locally</summary><p>No server renderer is connected. Use the installed CLI and a separate Blender runtime:</p><pre>arc-science molecule-render source.cif --antibody A,B --antigen C --assembly 1 --output ./new-candidate --blender-python /path/to/blender-env/bin/python</pre><p className="muted">Defaults: 1400 px · 96 samples · seed 23. Output must be a new directory.</p></details>
    </aside>
