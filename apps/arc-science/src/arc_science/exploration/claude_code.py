...
every tool, MCP server, hook, plugin, skill and session persistence disabled, in an
empty private directory, under a scrubbed environment, a hard timeout and (on
Windows) a kill-on-close job object. Arc never sees the credential; the CLI performs
its own OAuth. What the flags do not provide is an OS network sandbox: the CLI's own
transport reaches Anthropic, which is exactly what mission egress consent authorizes,
...
CONTRACT_VERSION = 'arc-claude-code-1'
...
class ClaudeCodeAgent:
    """Planner and reconciliation seats over `claude -p`; no visual seat."""
...
def observed_model(envelope, requested):
    """Exactly one CLI-reported model, and it must be the one configured."""
...
async def auth_status(command, environment=None, timeout=20.0):
    """Cost-free readiness: the CLI's own login state, never an inference."""
    try:
        process = await asyncio.create_subprocess_exec(
            *[str(part) for part in command], 'auth', 'status', '--json',
        ...
        return {'logged_in': False, 'auth_method': 'unknown'}


async def version(command, environment=None, timeout=20.0):
    try:
        process = await asyncio.create_subprocess_exec(
            *[str(part) for part in command], '--version',
            stdin=asyncio.subprocess.DEVNULL, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE,
            env=scrubbed_environment(environment))
        stdout, _ = await asyncio.wait_for(process.communicate(), timeout)
